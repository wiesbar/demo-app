...
Spring Boot 4.0.5 + Kotlin 2.2.21 on JDK 21, built with Gradle (Kotlin DSL). Uses Spring Boot's new `spring-boot-starter-webmvc` (servlet MVC) and Jackson via `tools.jackson.module:jackson-module-kotlin` (Jackson 3.x coordinates — not `com.fasterxml.jackson`). Persistence/search uses Spring Data Elasticsearch with the Elastic Java client 8.x; integration tests spin up a real Elasticsearch via the Testcontainers `elasticsearch` module.
...
- `example` — `DemoApplication` (the `@SpringBootApplication` entry point in `DemoApplication.kt`). Exposes `@Bean` factories that wire the calculator (`calculator()` → `DefaultArithmeticExpressionCalculator`) and the catalog stack (`semanticScorer()`, `productSerializer(...)`, `productQueryBuilder(...)`, `furnitureRepository(...)` — the latter binds `Dispatchers.IO` as the coroutine dispatcher). Keeping the wiring here means `example.calculator` and `example.catalog` stay free of Spring annotations.
...
- `example.catalog` — pure (non-Spring) furniture catalog + search engine. All types are `internal` (the `Category` and `UnitOfMeasure` enums are public because they appear on public DTOs).
